...
from transformers import AutoTokenizer, AutoModelWithLMHead, AutoModelForCausalLM, BitsAndBytesConfig
#tokenizer = AutoTokenizer.from_pretrained('../PretrainModels/checkpoint-1800/')
#tokenizer = AutoTokenizer.from_pretrained('tiiuae/falcon-7b-instruct')

#bnb_config = BitsAndBytesConfig(load_in_4bit=True,bnb_4bit_quant_type="nf4",bnb_4bit_compute_dtype=torch.float16,bnb_4bit_use_double_quant=True,)
#TestModel = AutoModelForCausalLM.from_pretrained("/export/share/lifu/llama_hf/Llama-2-70b-chat-hf", quantization_config=bnb_config, device_map="auto", trust_remote_code=True,)

"""
TestModel = AutoModelForCausalLM.from_pretrained(
            "/export/share/lifu/llama_hf/Llama-2-7b-chat-hf",
            device_map='auto',
...
def postprocess_next_token_scores(
        ...
        if no_repeat_ngram_size > 0:
            ...
            #print('banned_batch_tokens', banned_batch_tokens)
...
def generate(
        ...
        prefix_reward_prompts:Optional[torch.LongTensor]=None,
        choice_tokens:Optional[torch.LongTensor]=None,
        alpha: Optional[float] = None,
        ICL_length: Optional[int] = None,
    ...
    ICL_length= ICL_length if ICL_length is not None else 0
...
def _generate_no_beam_search(
    ...
    while cur_len < max_length:
        ...
        if self.config.model_type=='chatglm':
            if cur_len==init_len:
                model_inputs = self.prepare_inputs_for_generation(
                        input_ids, past_key_values=past_key_values, attention_mask=attention_mask, is_first_forward=True)
            else:
                model_input = self.prepare_inputs_for_generation( input_ids, past_key_values=past_key_values, attention_mask=attention_mask, is_first_forward=False)
        ...
        if do_sample:
            ...
            #print(torch.argmax(scores, dim=-1)[0])
            #print(next_tokens[0,0])

            ## -0.3 --> p = 0.5
            ## -0.5 --> p = 0.3
            # (next_scores[0,0].float() < -0.5 )
            if (next_scores[0,0].float() < -0.3 ) and  (look_goals!=None) and ((cur_len - init_len) > 2) and ( (cur_len - init_len + 1) % model_specific_kwargs['GuideFrquency']==0):
                ...
                for sentno in range(batch_size):
                    ...
                    temp_input_ids = torch.cat([input_ids[sentno].unsqueeze(0).expand(num_cands, -1), next_tokens[sentno].unsqueeze(-1)], dim=-1)

                    temp_attention_mask= attention_mask[sentno].unsqueeze(0).expand(num_cands, -1)
                    ...
                    #  TestModel, or self
                    ## do not use cache, use ICL_length or use init_len!!!
                    if choice_tokens==None:
                         future_state = _evalate_reverse_self(self, temp_input_ids,  ICL_length,
                                                   temp_look_goals, past_key_values, num_cands, temp_attention_mask,
                                                    temp_position_ids, self._reorder_cache, **decode_kwargs)
                    else:
                         future_state = _evalate_reverse_self_YoN(self, temp_input_ids,  ICL_length,
                                                   temp_look_goals, prefix_reward_prompts, choice_tokens, num_cands,
                                                   temp_attention_mask, temp_position_ids, self._reorder_cache, **decode_kwargs)

        
                    ## normalize score
                    ########future_state = future_state - torch.max(future_state)

                    #score_thres_mask = torch.le(next_token_prob_only[sentno], 0.5)
                    #print(next_token_prob_only[sentno])

                    #next_scores_clone[sentno] += alpha*math.tanh((cur_len - init_len )/5.0)*future_state #*score_thres_mask
                    ## here multiply length becase the beam score is also related to length
                    #print(next_scores[sentno], future_state)
                    #next_scores[sentno] += alpha*future_state

                    ## (torch.max(future_state) >= reward)
                    if  (torch.max(future_state)- future_state[0] > 0.05) :
                        next_scores[sentno] += alpha*future_state
                        #reward = torch.max(future_state)
                        #print(reward)
                        #print(future_state)
                ...
                #print('new', next_token)
                #next_token = torch.argmax(next_scores, dim=-1)

            else:
                # Greedy decoding
                next_token = torch.argmax(scores, dim=-1)
                #print('new greedy', next_token)
        ...
        # _, next_tokens = torch.topk(scores, 5, dim=1, largest=True, sorted=True)

        #print(use_cache, cur_len, tokens_to_add, next_tokens)
        if eos_token_id is not None:
...
def _evalate_reverse_self(
    ...
    for ii in range(total_constraint):
       ...
       #print(phrase_input_ids.size(), input_ids.size())
...
def _evalate_reverse_self_YoN(
    ...
    prefix_reward_prompts = prefix_reward_prompts[None,:].expand(batch_size, -1)

    for ii in range(total_constraint):
       phrase_input_ids = input_ids.new(look_goals[ii])[None, :].expand(batch_size, -1)
       phrase_input_ids = torch.cat([prefix_reward_prompts, input_ids[:, init_len:], phrase_input_ids], dim=-1)

  
       with torch.no_grad():
          follow_logits = self(input_ids=phrase_input_ids, labels=phrase_input_ids)[1]
       #print(follow_logits)
       logits = F.softmax(follow_logits[:, -1], dim=-1)
       logits = logits[:, choice_tokens]
       
       logits = logits / logits.sum(-1, keepdim=True)

       look_ahead_scores += torch.log(logits[:,0])
    ...
    while cur_len < max_length:
        ...
        if self.config.model_type=='chatglm':
            if cur_len==init_len:
                ...
                model_inputs = self.prepare_inputs_for_generation(
                        input_ids, past_key_values=past_key_values, attention_mask=attention_mask, is_first_forward=False)
        ...
        #print(model_inputs['position_ids'].size())
        ...
        #        scores, top_k=top_k, top_p=0.95, min_tokens_to_keep=2
        #    )

        # udpate --lifu
        scores = postprocess_next_token_scores(
            scores=scores,
            input_ids=input_ids, #[:, ICL_length:],
            no_repeat_ngram_size=no_repeat_ngram_size,
            bad_words_ids=bad_words_ids,
            cur_len=cur_len,
            min_length=min_length,
        ...
        #print('scores', scores.size(), "input_ids", input_ids.size(), 'batch_size', batch_size, 'num_beams', num_beams) 
        #print(no_repeat_ngram_size, input_ids[0], scores[0])
        ...
        if do_sample:
            ...
            #print(next_scores[0,:vocab_size])
            next_scores, next_tokens = torch.topk(next_scores, 2 * num_beams, dim=1, largest=True, sorted=True)

            #print(cur_len,  next_tokens)
            ...
            next_token_prob_only = torch.gather(probs.view(batch_size, num_beams * vocab_size), -1, next_tokens)
            #print(next_token_prob_only.size())
            if (look_goals!=None) and ((cur_len - init_len) > 2) and ( (cur_len - init_len + 1) % model_specific_kwargs['GuideFrquency']==0):
                #next_scores_clone = next_scores.clone().detach()
                ...
                for sentno in range(batch_size):
                    ...
                    #print(input_ids.size(), temp_input_ids.size())
                    ...
                    if choice_tokens==None:
                         ...
                         future_state = _evalate_reverse_self_YoN(self, temp_input_ids,  ICL_length,
                                                   temp_look_goals, prefix_reward_prompts, choice_tokens, 2 * num_beams, 
                                                   temp_attention_mask, temp_position_ids, self._reorder_cache, **decode_kwargs)
        ...
        for batch_idx in range(batch_size):
            ...
            for beam_token_rank, (beam_token_id, beam_token_score, beam_token_score_clone) in enumerate(
                ...
                if (eos_token_id is not None) and (token_id.item() == eos_token_id):
                    ...
                    ## changed above, beam score is added.  --lifu
                else:
                    # add next predicted token since it is not eos_token,
                    ## constraint score is not added. --lifu

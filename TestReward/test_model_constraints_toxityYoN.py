...
def main():
    ...
    print('answer choics: ', choice_tokens)
    ...
    if 'nli' in args.model_name:
        ...
        #goal_line = tokenizer.tokenize( tokenizer.eos_token + ' Will it a polite, respectful, and reasonable comment?')
        # goal_line = tokenizer.tokenize( tokenizer.eos_token + ' Will this be a rude, disrespectful, or unreasonable comment?')
        goal_line = tokenizer.tokenize( tokenizer.eos_token + ' Will this be a rude, disrespectful, or unreasonable comment? Answer Yes or No')
        if args.mc: 
           goal_line = []
           goal_line.append( tokenizer.tokenize(tokenizer.eos_token + 'Will it a rude, disrespectful, or unreasonable comment? Yes'))
           goal_line.append(tokenizer.tokenize( tokenizer.eos_token + 'Will it a rude, disrespectful, or unreasonable comment? No'))
...
def evalate_reverse_self2(
    ...
    for ii in range(total_constraint):
       ...
       #follow_log_prob = F.log_softmax(follow_logits[:,-1,:], dim=-1)
       
       follow_logits = follow_logits[:, -1,]
       target_Y = look_goals[:, 0, -1]#.shape[-1]
       target_N = look_goals[:, 1, -1]#.shape[-1]
    
       look_ahead_score1 = follow_logits[torch.arange(batch_size), target_Y]
       """   
       #look_ahead_score2 = follow_logits[torch.arange(batch_size), target_N]
    
       #S = torch.stack((look_ahead_score1, look_ahead_score2), dim=1)

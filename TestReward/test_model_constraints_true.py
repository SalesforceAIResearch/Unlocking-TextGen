...
import os
os.environ['TRANSFORMERS_CACHE'] = '/export/home/tmp'
...
def main():
    ...
    parser.add_argument("--reverse", action="store_true", help="reverse reward is used.")
    ...
    """
    if args.split=='vitc':
        f =  open('../data/true/vitc_ranking1000.csv', mode= 'r')
    else:
        f =  open('../data/true/fever_ranking1000.csv', mode= 'r')
    ...
    for line in csvFile:
           ...
           #if label==0: 
           if args.YoN:
               #input_lines_A.append(tokenizer.tokenize(f"Claim:{a}\n\n"))
               #input_lines_B.append(tokenizer.tokenize(f"Claim:{b}\n\n"))
               input_lines_A.append(tokenizer(f"Claim:{a}\n\n")['input_ids'])
               input_lines_B.append(tokenizer(f"Claim:{b}\n\n")['input_ids'])
    
               YoN_prompts = f"Document: {g}\n\nQuestion: Is the above claim supported by the above document? Answer with Yes or No.\n\nAnswer:"
               #goal_lines.append(tokenizer.tokenize( tokenizer.eos_token + YoN_prompts))
               goal_lines.append(tokenizer.tokenize( YoN_prompts))

           else:
               #input_lines_A.append(tokenizer.tokenize(a))
               #input_lines_B.append(tokenizer.tokenize(b))
               input_lines_A.append(tokenizer( a)['input_ids'])
               input_lines_B.append(tokenizer( b)['input_ids'])
               #goal_lines.append(tokenizer.tokenize( tokenizer.eos_token + g))
               goal_lines.append(tokenizer.tokenize(g))
           
           #input_lines_A.append(tokenizer.tokenize(tokenizer.eos_token + a))
           #input_lines_B.append(tokenizer.tokenize(tokenizer.eos_token + b))
    ...
    if 'nli' in args.model_name:
        ...
        """
        if 'Mixtral-8x7B' in args.model_name:
            """
        ...
        model = LlamaForCausalLMFlash.from_pretrained(
            args.model_name,
            torch_dtype=torch.bfloat16, device_map="auto", offload_folder="offload", offload_state_dict = True,
        )
        #if torch.cuda.device_count() > 1:
        #    model = tp.tensor_parallel(model, [i for i in range(torch.cuda.device_count())])
        """      
    ...
    while next_i < len(input_lines_A):
            ...
            if 'nli' in args.model_name:
                ...
                for premise1, premise2, hypothesis in zip(buf_a, buf_b, goal_chunk):
                   ...
                   if args.reverse:
                       buf_a = [ [PAD_ID] + x + [PAD_ID] * (max_goal_len - len(x)) for x in buf_a]
                   else:
                   ...
                   if args.reverse:
                       buf_b = [ [PAD_ID] + x + [PAD_ID] * (max_goal_len - len(x)) for x in buf_b]
                   else:
                   ...
                   if args.mc:
                       ...
                       goal_buf = [ [tokenizer.pad_token_id] + x0 + [PAD_ID] * (max_goal_len - len(x0)) for x in goal_chunk for x0 in x]
                       ...
                       if args.reverse:
                           ...
                           goal_buf = [ [PAD_ID] + x + [PAD_ID] * (max_goal_len - len(x)) for x in goal_chunk]
                   #print(input_ids_a[0])
                   look_goals = torch.stack([torch.from_numpy(np.array(x)) for x in goal_buf])
                   if args.mc:
                   ...
                   if args.YoN:
                       score_a = evalate_reverse_self2(self=model, input_ids=input_ids_a, look_goals=look_goals, pad_token_id=PAD_ID, choice_tokens=choice_tokens)
                       score_b = evalate_reverse_self2(self=model, input_ids=input_ids_b, look_goals=look_goals, pad_token_id=PAD_ID, choice_tokens=choice_tokens)
               
                   else:
                       if args.reverse:
                           score_a = evalate_reverse_self(self=model, input_ids=look_goals,look_goals=input_ids_a, batch_size=step_len)
                           score_b = evalate_reverse_self(self=model, input_ids=look_goals,look_goals=input_ids_b, batch_size=step_len)
                       else:
                   ...
                   result = torch.gt(score_a, score_b).cpu().numpy().tolist()
                   
                   if result[0] < 1 :
                       #print(premise1, premise2, hypothesis)

                       print(tokenizer.decode(look_goals[0], skip_special_tokens=True))
                       print(tokenizer.decode(input_ids_a[0], skip_special_tokens=True))
                       print(tokenizer.decode(input_ids_b[0], skip_special_tokens=True))
                       print(score_a.cpu().numpy(), score_b.cpu().numpy())
                       print("*"*10)
                   #print(result.cpu().numpy().tolist())
                   acc = acc + result
...
def evalate_reverse_self2(
    ...
    for ii in range(total_constraint):
       ...
       look_ahead_scores += logits[:,0]
    
       # previous
       #logits = logits[:, choice_tokens]

       #look_ahead_scores += logits.softmax(-1)[:,0]
